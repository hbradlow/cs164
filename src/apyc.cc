...
const string LIB_SUFFIX = "/lib/apyc";

const string PRELUDE_NAME = LIB_SUFFIX + "/prelude.py";
const string RUNTIME = LIB_SUFFIX + "/runtime.cc";
const string GCC = "g++";
/** The extension to be used for generated code.  The value here is
 *  for C++.  Change it to .s (assembler) or .c as appropriate. */
const string CODE_SUFFIX = ".cc";

/** Template for the name of a temporary code file when executable
 *  code is produced. */
const string TEMPORARY_TEMPLATE = "/tmp/apycXXXXXX";
...
static void
openStdout (const string& output)
{
...
static void
makeExecutable (const string& source, const string& executable)
{
    string command = GCC + " -g -I " + execPrefix + LIB_SUFFIX + 
...
main (int argc, char* argv[])
{
    ...
    string prefix = string(infile, 0, infile.size ()-3);

    execPrefix = argv[0];
    size_t lastSeg = execPrefix.find_last_of ("/\\");
    execPrefix.erase (lastSeg == string::npos ? 0 : lastSeg+1);
