...
protected:
    ...
    void generateArgs(ostream& out, int i, int c_i_, AST_Ptr c)
    {
        ...
        writeComment(out,i,"Add it to the closures frame's frame");
        writeIndented(out,i);
        child(0)->innerCodeGen(out,i);
        out << "_dyn_" << local_count << child(0)->getDecl()->getIndex();
        out << "->frame->setVar(";
...
protected:
    ...
    void defCodeGen(ostream& out,int i){
        local_count = global_count++;
        writeComment(out,i,"----------------------------start--------------------");
        writeComment(out,i,"AND Function stuff");

        writeIndented(out,i);
        out << "Object*";
        out << " AND_" << local_count;
        out << "(";
        out << "Frame* frame";
        out << "){\n";
